...
public static class WebBridge
{
    ...
    public static string BuildOverlayPayload(SystemSnapshot snapshot, AggregateSample[] history, VisibilitySettings visible)
    {
        var payload = new
        {
            ...
            visible = new { cpu = visible.Cpu, gpu = visible.Gpu, ram = visible.Ram, temps = visible.Temps },
            history = new
            {
    ...
    /// </summary>
    public static string BuildDetailPayload(SystemSnapshot snapshot, AggregateSample[] history, IReadOnlyList<ProcessSample>? processes)
    {
        var payload = new
        {
            type = "detail",
    ...
    private static double[] Series(AggregateSample[] history, int points, Func<AggregateSample, double> selector)
    {
        int skip = Math.Max(0, history.Length - points);
        var result = new double[history.Length - skip];
        for (int i = 0; i < result.Length; i++)
            result[i] = Round(selector(history[skip + i]));
        return result;
    }
